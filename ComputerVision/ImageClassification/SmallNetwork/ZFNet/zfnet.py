...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        filepath='model_zf_net.h5', save_weights_only=True, monitor='val_loss', save_best_only=True),
...
def ZFnet():
    ...
    x = tf.keras.layers.Conv2D(
        64, (7, 7), strides=(2, 2), activation='relu')(inputs)
...
if __name__ == '__main__':
    model = ZFnet()
    model.summary()
    tf.keras.utils.plot_model(model, to_file=ZFnet.__name__+'.png')
    ...
    model.compile(loss='sparse_categorical_crossentropy',
                  optimizer=tf.optimizers.Adam(lr=0.001), metrics=['accuracy', tf.keras.metrics.TopKCategoricalAccuracy(5)])
    model.summary()
    model.fit(train_ds,
              epochs=50,
