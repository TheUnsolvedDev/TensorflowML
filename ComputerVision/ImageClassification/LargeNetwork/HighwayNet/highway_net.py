...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        filepath='model_highway_net.h5', save_weights_only=True, monitor='val_loss', save_best_only=True),
...
def highway_network():
    ...
    x = tf.keras.layers.Conv2D(
        32, (3, 3), padding='same', activation='relu')(inputs)
    x = tf.keras.layers.Conv2D(
...
if __name__ == '__main__':
    input_shape = (32, 32, 3)
    num_classes = 10
    model = highway_network()
    model.summary()
    tf.keras.utils.plot_model(
        model, to_file=highway_network.__name__+'.png', show_shapes=True)
