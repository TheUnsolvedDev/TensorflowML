...
def train_model(train, val, epochs=ITERATION, batch_size=BATCH_SIZE):
    ...
    try:
        gen.load_weights('ls_gan_weights/generator_ls_gan.h5')
        disc.load_weights('ls_gan_weights/discriminator_ls_gan.h5')
    except FileNotFoundError:
        os.makedirs('ls_gan_weights',exist_ok=True)
    ...
    for epoch in range(epochs+1):
        ...
        if epoch % EVERY_STEP == 0:
            ...
            if loss <= min_loss_gen:
                disc.save_weights('ls_gan_weights/discriminator_ls_gan.h5')
                gen.save_weights('ls_gan_weights/generator_ls_gan.h5')
                loss = min_loss_gen
