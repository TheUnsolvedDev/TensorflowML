...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        filepath='model_deep_nn_gan.h5', save_weights_only=True, monitor='loss', save_best_only=True),
...
def make_generator():
    ...
    x = tf.keras.layers.Dense(
        IMAGE_SHAPE[0]*IMAGE_SHAPE[1], activation='sigmoid')(x)
...
class DeepNNGAN(tf.keras.Model):
    def __init__(self,):
        super(DeepNNGAN, self).__init__()
