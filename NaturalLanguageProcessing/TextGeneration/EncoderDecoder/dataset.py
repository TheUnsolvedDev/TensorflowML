...
class Dataset:
    ...
    def _build_or_load_vectorizer(self):
        if os.path.exists(self.vocab_path):
            ...
            print(f"Building vocabulary from {self.data_path}...")
    ...
    def get_dataset(self, for_training=True):
        ...
        def prepare_seq2seq_data(sequence):
            input_seq = sequence[:-1]
            target_seq = sequence[1:]
            decoder_input = tf.concat([[0], target_seq[:-1]], axis=0)
            return input_seq, decoder_input, target_seq

        ds = raw_ds.map(prepare_seq2seq_data,
