...
writer = SummaryWriter('Atari/BreakOut')
...
class QNetwork:
    ...
    def update_weights(self, tau: float = 0.99) -> None:
    ...
    def get_model(self) -> tf.keras.Model:
        ...
        x = tf.keras.layers.Conv2D(
            filters=5, kernel_size=(3, 3), activation='relu')(inputs)
        x = tf.keras.layers.MaxPool2D(pool_size=(
            3, 3), strides=2, padding='valid')(x)
        x = tf.keras.layers.Conv2D(
            filters=5, kernel_size=(3, 3), activation='relu')(x)
        x = tf.keras.layers.MaxPool2D(pool_size=(
            3, 3), strides=2, padding='valid')(x)
...
class Policy:
    ...
    def epsilon_greedy(model, state, epsilon=0.1):
        ...
        if np.random.rand() < epsilon:
            return np.random.randint(NUM_ACTIONS, size=(state.shape[0]))
        else:
...
class DQN_Agent:
    def __init__(self, strategy='epsilon_greedy'):
        self.network = QNetwork(input_shape=(64, 64, 4), output_shape=4)
    ...
    def update(self, states, actions, rewards, next_states, dones):
        with tf.GradientTape() as tape:
            ...
            target = tf.reduce_max((1.0-dones)*target, axis=1)
            current = self.source(states)
            current = tf.reduce_sum(
                current*tf.one_hot(actions, NUM_ACTIONS), axis=1)
            loss = self.mse_loss(
...
def wrapper(env):
    env = gym.wrappers.AtariPreprocessing(
        env, frame_skip=1, screen_size=64, grayscale_obs=True, scale_obs=True)
    env = gym.wrappers.FrameStack(env, 4)
    env = gym.wrappers.TransformObservation(
        env, lambda x: np.array(x).reshape((64, 64, 4)))
    return env
...
class TrainEnv:
    def __init__(self, name: str):
        ...
        print("Environment shape:", self.env.reset()[0].shape)
...
class TestEnv:
    def __init__(self, name: str):
        ...
        self.env = gym.wrappers.RecordVideo(
            self.env, video_folder='Atari/BreakOut', episode_trigger=lambda x: True)
        print("Environment shape:", self.env.reset()[0].shape)
