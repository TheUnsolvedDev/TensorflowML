...
def train(environment, agent, epsilons, step, num_steps=1000):
...
def session(num_steps=NUM_TRAINING_STEPS):
    ...
    agent = DuelingDoubleDeepQNetworkAgent(input_shape=(
        IMG_SIZE[0], IMG_SIZE[1], NUM_CHANNELS), num_actions=NUM_ACTIONS, num_envs=NUM_ENVS)
    ...
    for _ in tqdm.tqdm(range(1, num_steps+1, num_steps_to_train_for)):
        agent = train(environment=env, epsilons=epsilons, step=_,
                      agent=agent, num_steps=num_steps_to_train_for)
        test_rewards = [test(environment=test_env, agent=agent,
                             num_steps=500) for i in range(10)]
