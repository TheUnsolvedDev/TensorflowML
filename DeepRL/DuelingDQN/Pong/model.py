...
def DuelingQModel(input_shape=(IMG_SIZE[0], IMG_SIZE[1], NUM_CHANNELS), num_actions=NUM_ACTIONS):
    ...
    x = tf.keras.layers.Conv2D(32, (8, 8), strides=(4, 4), activation='selu')(x)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=1)(x)
    x = tf.keras.layers.Conv2D(64, (4, 4), strides=(2, 2), activation='selu')(x)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(x)
    x = tf.keras.layers.Flatten()(x)
    # Dueling architecture
    # Value stream
    v = tf.keras.layers.Dense(64, activation='selu')(x)
    v = tf.keras.layers.Dense(1)(v)

    # Advantage stream
    a = tf.keras.layers.Dense(64, activation='selu')(x)
