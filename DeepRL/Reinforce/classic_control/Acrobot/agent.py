    ...
    def learn(self, states, actions, returns):
        with tf.GradientTape() as policy_tape, tf.GradientTape() as baseline_tape:
            ...
            action_probs = tf.reduce_sum(
                action_probs * tf.one_hot(actions, self.output_shape[0]), axis=1)
            action_probs = tf.clip_by_value(action_probs, 1e-10, 1.0)
            log_probs = tf.math.log(action_probs)
            baseline_values = tf.squeeze(
                self.baseline_model(states), axis=-1)  # (batch_size,)
            advantages = returns - baseline_values
            policy_loss = -tf.reduce_mean(log_probs * advantages) + 0.01 * tf.reduce_mean(log_probs)
            baseline_loss = tf.reduce_mean(tf.square(advantages))

        policy_grads = policy_tape.gradient(
            policy_loss, self.model.trainable_variables)
        policy_grads = [tf.clip_by_value(grad, -5.0, 5.0)
                        for grad in policy_grads]
        self.optimizer.apply_gradients(
            zip(policy_grads, self.model.trainable_variables))

        baseline_grads = baseline_tape.gradient(
            baseline_loss, self.baseline_model.trainable_variables)
        baseline_grads = [tf.clip_by_value(
            grad, -5.0, 5.0) for grad in baseline_grads]
