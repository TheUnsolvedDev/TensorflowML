...
def train(environment, agent, step, writer, num_steps=1000):
    ...
    done_flag = np.zeros((NUM_ENVS,))
    pointer_flag = np.zeros((NUM_ENVS,))
    batched_states,batched_actions,batched_rewards = [],[],[]
    ...
    for _ in range(0, num_steps):
        ...
        for i in range(environment.num_envs):
            if done[i] or agent.buffer.pointers[i] > N_STEP:
                done_flag[i] = 1
                pointer_flag[i] = agent.buffer.pointers[i]
                states, actions, rewards, next_state, done_ = agent.buffer.pop_trajectories_n_step(i)
                returns = agent._compute_returns(rewards, next_state, done_)
                batched_states.append(states)
                batched_actions.append(actions)
                batched_rewards.append(returns)
                    
        if np.all(done_flag) or np.all(agent.buffer.pointers > N_STEP):
            done_flag.fill(0)
            pointer_flag.fill(0)
            stacked_states = np.concatenate(batched_states,axis=0)
            stacked_actions = np.concatenate(batched_actions,axis=0).reshape(-1, )
            stacked_rewards = np.concatenate(batched_rewards,axis=0).reshape(-1, )
            batched_states, batched_actions, batched_rewards = [], [], []
            loss = agent.learn(stacked_states, stacked_actions, stacked_rewards)
            with writer.as_default():
                tf.summary.scalar('loss', loss.numpy(), step=current_step)
            print(
